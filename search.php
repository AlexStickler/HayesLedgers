...
<body>
  ...
  <br><br><br><br>
    ...
    $sql = "SELECT * FROM l1 WHERE name_first LIKE '%$name_first%' OR name_first IS NULL
    AND name_middle LIKE '%$name_middle%' OR name_middle IS NULL
    AND name_last LIKE '$name_last%' OR name_last IS NULL
    AND interment_at LIKE '%$interment_at%' OR interment_at IS NULL
    AND cause_of_death LIKE '%$cause_of_death%' OR cause_of_death IS NULL
    AND age_years LIKE '%$age_years%' OR age_years IS NULL
    AND occupation LIKE '%$occupation%' OR occupation IS NULL
    AND certifying_physician LIKE '%$certifying_physician%' OR certifying_physician IS NULL
    AND marriage_status LIKE '%$marriage_status%' OR marriage_status IS NULL
    AND date_of_death LIKE '%$date_of_death%' OR date_of_death IS NULL
    AND date_of_funeral LIKE '%$date_of_funeral%' OR date_of_funeral IS NULL
    AND place_of_death LIKE '%$place_of_death%' OR place_of_death IS NULL
    AND total_footing_of_bill LIKE '%$total_footing_of_bill%' OR total_footing_of_bill IS NULL
    AND charge_to LIKE '%$charge_to%' OR charge_to IS NULL ORDER BY age_years ASC";
